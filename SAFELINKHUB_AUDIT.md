...
> ✅ **CORRIGÉ le 8 août 2026 (v179)** — les sept sont fermées. Cinq helpers ont été déplacés dans des modules « plain » que le webhook et le cron importent (`wallet/balance.ts`, `wallet/topup-confirmation.ts`, `safecoin/topup-confirmation.ts`, `mikrotik/mndp-sync.ts`) ; `listBridges` est supprimée (code mort) ; `getFloatBalanceCents` est désexportée ; `syncMndpAnnouncements` reste une action — un composant client l'appelle — mais devient une façade gardée SANS paramètre, qui dérive l'organisation de la session. Re-scan exhaustif après correction : **170 actions, 159 gardées, 11 non gardées — toutes sur la liste blanche légitime**. L'invariant est désormais **testé** (`test/server-action-guards.test.mjs`) : toute nouvelle action exportée sans garde fait échouer la suite.
