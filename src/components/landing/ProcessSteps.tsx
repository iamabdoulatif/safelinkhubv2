...
export default function ProcessSteps({ dict }: { dict: Dictionary }) {
  return (
    <section aria-label={dict.process.aria} className="border-b border-line bg-paper py-16 sm:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <SectionIntro
          eyebrow={dict.process.eyebrow}
          title={dict.process.title}
          marker={dict.process.mark}
        />
        <ol className="stagger mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {processSteps.map((_, i) => {
            const step = dict.content.processSteps[i];
            return (
