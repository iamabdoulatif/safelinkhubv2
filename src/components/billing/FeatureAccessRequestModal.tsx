...
// TEMPORAIRE — modal de demande d'accès à une fonctionnalité verrouillée
// (router_link / remote_access). S'affiche quand un utilisateur non autorisé
// tente d'utiliser la fonctionnalité. Sans paiement : l'utilisateur envoie une
// simple demande (message facultatif) → l'admin est notifié (email + WhatsApp)
// et approuve depuis /admin/authorizations.
...
} from "@/lib/billing/feature-access-config";
import { requestFeatureAccess } from "@/lib/billing/feature-access-actions";

export default function FeatureAccessRequestModal({
  open,
  onClose,
  feature,
  latestStatus,
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await requestFeatureAccess(fd);
      if ("error" in res) {
        setError(res.error);
        return;
      }
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-ink">{label} — accès à débloquer</h2>
        </div>
            ...
            <div className="flex items-start gap-2 rounded-md bg-ok-soft p-3">
              ...
              <div className="text-sm text-ok">
                ...
                <p className="mt-1">
                  ...
                  {done.emailSent ? " Un email lui a été envoyé." : ""} Vous pouvez aussi le
                  prévenir directement via WhatsApp.
                </p>
                <a
            ...
            <div className="mt-4">
              <label className="block">
                <span className="text-xs font-medium text-ink-soft">
                  Message à l&apos;administrateur (facultatif)
                </span>
                <textarea
                  value={note}
