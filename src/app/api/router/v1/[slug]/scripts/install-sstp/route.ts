...
import { allocateSstpPeer } from "@/lib/mikrotik/relay";
import { hashToken } from "@/lib/mikrotik/install-token";
import { relayConnectTo } from "@/lib/mikrotik/openvpn-install-script";
import { buildSstpInstallScript } from "@/lib/mikrotik/sstp-install-script";
import { TUNNEL_SUBNETS } from "@/lib/mikrotik/tunnel-methods";
...
export async function GET(
  ...
  let peer;
  try {
    peer = await allocateSstpPeer(`${org.slug}-${router.name}`);
  } catch (err) {
    return new Response(
      `Could not allocate SSTP tunnel: ${err instanceof Error ? err.message : "unknown error"}`,
      { status: 502 },
    );
  }
  ...
  // Une IP, jamais un nom : c'est l'absence de SNI qui fait router la
  // connexion vers le serveur SSTP (point 2 de sstp-install-script.ts).
  ...
  const callbackUrl = new URL(
    `/api/router/v1/${org.slug}/scripts/install-sstp/installed`,
