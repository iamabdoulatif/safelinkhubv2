...
/** Teinte d'une pastille d'état — jetons de la charte, pas de pastels bruts.
 *  Même vocabulaire que les pastilles KYC : aplat plein pour l'état terminal
 *  souhaité, fond d'erreur pour le refus, argile pour l'attente. */
function statusPill(status: string) {
  if (status === "approved") return "bg-ok text-white";
  if (status === "pending") return "bg-clay text-warn";
...
/**
 * Tuile « à traiter ». Trois accents seulement, et chacun dit quelque chose :
 * ambre = une décision est attendue de vous, encre = à surveiller sans agir,
 * rouge = une vente perdue. Les pastels bruts d'avant (amber-50, blue-50,
 * red-50) étaient les seules couleurs hors charte de l'administration.
 *
 * `href` absent = aucune destination : la tuile ne prend alors NI le survol NI
 * la flèche, pour ne pas promettre un clic qui ne mène nulle part.
 */
function AlertTile({
  count,
  title,
  ...
  // Rien à signaler : la tuile disparaît. Une pastille « 0 en attente » en
  // couleur d'alerte apprend à ignorer la rangée entière.
  if (count <= 0) return null;

  const bords = { warn: "border-l-warn", ink: "border-l-ink", err: "border-l-err" } as const;
  const teintes = { warn: "text-warn", ink: "text-ink", err: "text-err" } as const;
  const contenu = (
    <>
      <Icon className={`tile-hover-icon h-5 w-5 shrink-0 ${teintes[accent]}`} aria-hidden="true" />
      <span className="min-w-0 flex-1">
        <strong className="block text-sm text-ink">
          <span className="tabular-nums">{count}</span> {title}
        </strong>
        <span className="text-xs text-ink-soft">{hint}</span>
      </span>
      {href && (
        <ArrowUpRight className="h-4 w-4 shrink-0 text-ink-soft" aria-hidden="true" />
      )}
    </>
  );

  const classe = `flex flex-1 basis-64 items-center gap-3 border border-line border-l-4 bg-paper p-3 rounded-xl ${bords[accent]}`;
  return href ? (
    <Link href={href} className={`tile-hover ${classe}`}>
      {contenu}
    </Link>
  ) : (
...
export default function PlatformAnalyticsView({
  ...
  return (
    <div className="animate-fade-in-up space-y-6">
      ...
      {/* Rangée « à traiter » : en flex plutôt qu'en grille de trois, pour
          qu'une tuile seule occupe la largeur au lieu de laisser deux
          cellules vides à côté d'elle. */}
      <div className="flex flex-wrap gap-3 empty:hidden">
        <AlertTile
          count={report.kpis.pendingCount}
          title="en attente"
        ...
        />
        <AlertTile
          count={report.kpis.unconsumedApprovedCount}
          title="non activée(s)"
          hint="Paiements validés à suivre"
          icon={CircleAlert}
          accent="ink"
          href="/admin/vpn-access"
        />
        <AlertTile
          count={report.kpis.rejectedCount}
          title="refusée(s)"
          hint="Demandes non encaissées"
          icon={CircleAlert}
          accent="err"
        />
      </div>
