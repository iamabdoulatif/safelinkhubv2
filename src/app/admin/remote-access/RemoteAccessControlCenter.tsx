...
function MetricCard({
  ...
  return (
    <article className={`border-2 bg-paper p-4 ${toneClass}`}>
...
function RouterDetail({
  ...
  return (
    <section
      className={drawer ? "h-full overflow-y-auto bg-paper p-5 sm:p-6" : "border-2 border-line bg-paper p-5"}
    ...
    >
      ...
      <div className="mt-4 border border-line-soft bg-clay/50 px-3 py-2.5">
...
export default function RemoteAccessControlCenter({
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      {priorityRouter && (
        <section className="mt-5 flex flex-col gap-3 border-2 border-warn/40 bg-brand/10 p-4 sm:flex-row sm:items-center" aria-label="Action prioritaire">
          ...
          <button type="button" onClick={() => setIncidentOnly(true)} className="min-h-11 border-2 border-line bg-paper px-3 py-2 text-sm font-bold text-ink hover:bg-clay">Voir les incidents</button>
        </section>
      )}
      ...
      <section className="mt-8" aria-labelledby="fleet-heading">
        ...
        <div className="mt-4 grid gap-3 border-2 border-line bg-paper p-3 lg:grid-cols-[minmax(0,1fr)_160px_160px_auto_auto]">
          ...
          <label><span className="sr-only">Filtrer par état</span><select value={status} onChange={(event) => setStatus(event.target.value as ControlCenterFilters["status"])} className="min-h-11 w-full border border-line bg-paper px-3 text-sm text-ink"><option value="all">Tous les états</option><option value="online">En ligne</option><option value="attention">À vérifier</option></select></label>
          <label><span className="sr-only">Filtrer par tunnel</span><select value={method} onChange={(event) => setMethod(event.target.value as ControlCenterFilters["method"])} className="min-h-11 w-full border border-line bg-paper px-3 text-sm text-ink"><option value="all">Tous les tunnels</option><option value="wireguard">WireGuard</option><option value="openvpn">OpenVPN</option><option value="direct">Sans tunnel</option></select></label>
          <button type="button" aria-pressed={incidentOnly} onClick={() => setIncidentOnly((value) => !value)} className={`inline-flex min-h-11 items-center justify-center gap-2 border-2 px-3 text-sm font-bold ${incidentOnly ? "border-ink bg-ink text-paper" : "border-line bg-paper text-ink hover:bg-clay"}`}><Filter className="h-4 w-4" aria-hidden="true" /> Incidents</button>
          {hasFilters && <button type="button" onClick={resetFilters} className="inline-flex min-h-11 items-center justify-center gap-2 border-2 border-line bg-paper px-3 text-sm font-bold text-ink hover:bg-clay"><SlidersHorizontal className="h-4 w-4" aria-hidden="true" /> Effacer</button>}
        </div>

        {routers.length === 0 ? (
          <div className="mt-4 border-2 border-dashed border-line bg-paper px-6 py-14 text-center"><RouterIcon className="mx-auto h-8 w-8 text-ink-soft" aria-hidden="true" /><p className="mt-3 font-semibold text-ink">Aucun routeur configuré</p><p className="mt-1 text-sm text-ink-soft">Installez un tunnel pour connecter votre premier MikroTik.</p><div className="mt-5"><RemoteAccessTunnelDialog /></div></div>
        ) : filteredRouters.length === 0 ? (
          <div className="mt-4 border-2 border-dashed border-line bg-paper px-6 py-14 text-center"><Search className="mx-auto h-8 w-8 text-ink-soft" aria-hidden="true" /><p className="mt-3 font-semibold text-ink">Aucun routeur ne correspond à ces filtres</p><button type="button" onClick={resetFilters} className="mt-4 min-h-11 border-2 border-line bg-paper px-3 text-sm font-bold text-ink hover:bg-clay">Effacer les filtres</button></div>
        ) : (
          <div className="mt-4 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div>
              <div className="hidden overflow-hidden border-2 border-line bg-paper xl:block"><table className="w-full text-left text-sm"><thead className="border-b-2 border-line bg-clay"><tr className="font-mono text-[11px] font-semibold uppercase tracking-widest text-ink-soft"><th scope="col" className="px-4 py-3">Routeur</th><th scope="col" className="px-4 py-3">Tunnel</th><th scope="col" className="px-4 py-3">Accès</th><th scope="col" className="px-4 py-3">État</th><th scope="col" className="px-4 py-3"><span className="sr-only">Espace routeur</span></th></tr></thead><tbody>{filteredRouters.map((router) => <tr key={router.id} className={selectedRouter?.id === router.id ? "border-b border-line-soft bg-brand/10 last:border-0" : "border-b border-line-soft last:border-0 hover:bg-clay/50"}><td className="p-0"><button ref={(element) => { rowRefs.current[router.id] = element; }} type="button" onClick={() => selectRouter(router.id)} aria-current={selectedRouter?.id === router.id ? "true" : undefined} className="w-full px-4 py-3 text-left outline-none focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-ink"><span className="font-bold text-ink">{router.name}</span><span className="mt-1 block text-xs text-ink-soft">Dernier contact : {dateLabel(router.lastSyncAt)}</span></button></td><td className="px-4 py-3"><span className="inline-flex border border-line-soft bg-paper px-2 py-1 text-xs font-semibold text-ink">{connectionMethodLabel(router.connectionMethod)}</span></td><td className="px-4 py-3 tabular-nums text-ink">{router.activeForwards.length} actif{router.activeForwards.length > 1 ? "s" : ""}</td><td className="px-4 py-3"><RouterStatus router={router} /></td><td className="px-4 py-3 text-right"><Link href={`/admin/remote-access/${router.id}`} className="inline-flex min-h-9 items-center gap-1 border border-line px-2.5 text-xs font-bold text-ink hover:bg-clay">Gérer <ChevronRight className="h-3.5 w-3.5" aria-hidden="true" /></Link></td></tr>)}</tbody></table></div>
              <ul className="space-y-3 xl:hidden">{filteredRouters.map((router) => <li key={router.id} className={selectedRouter?.id === router.id ? "border-2 border-brand bg-brand/10" : "border-2 border-line bg-paper"}><button ref={(element) => { rowRefs.current[router.id] = element; }} type="button" onClick={() => selectRouter(router.id, true)} aria-current={selectedRouter?.id === router.id ? "true" : undefined} className="w-full p-4 text-left outline-none focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-ink"><span className="flex items-start justify-between gap-4"><span><span className="block font-bold text-ink">{router.name}</span><span className="mt-1 block text-xs text-ink-soft">{connectionMethodLabel(router.connectionMethod)} · {router.activeForwards.length} accès actif{router.activeForwards.length > 1 ? "s" : ""}</span></span><RouterStatus router={router} /></span></button><div className="border-t border-line-soft px-4 py-3"><Link href={`/admin/remote-access/${router.id}`} className="inline-flex min-h-9 items-center gap-1 text-xs font-bold text-ink hover:text-brand-deep">Ouvrir l’espace routeur <ChevronRight className="h-3.5 w-3.5" aria-hidden="true" /></Link></div></li>)}</ul>
            </div>
