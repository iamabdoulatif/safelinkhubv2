  ...
  return (
    <section className={embedded ? "bg-transparent p-1 sm:p-2" : "border border-line bg-paper p-5 rounded-xl sm:p-6"}>
      ...
      <form action={action} className="mt-5 grid gap-4 border border-line-soft bg-clay/40 p-4 lg:grid-cols-2 rounded-xl">
        ...
        <label className="text-sm font-medium text-ink">Routeur ciblé <select name="routerId" className="mt-1.5 w-full border border-line-soft bg-paper px-3 py-2.5 text-sm rounded-lg"><option value="">Tous les routeurs de l&apos;organisation</option>{routers.filter((router) => router.orgId === selectedOrg).map((router) => <option key={router.id} value={router.id}>{router.name}</option>)}</select></label>
        ...
        <label className="text-sm font-medium text-ink">Motif<select name="reason" defaultValue="promo" className="mt-1.5 w-full border border-line-soft bg-paper px-3 py-2.5 text-sm rounded-lg">{Object.entries(reasonLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        <label className="text-sm font-medium text-ink">Note obligatoire<textarea name="note" required rows={2} placeholder="Ex : Parrainage de la campagne juillet" className="mt-1.5 w-full resize-none border border-line-soft bg-paper px-3 py-2.5 text-sm rounded-lg" /></label>
