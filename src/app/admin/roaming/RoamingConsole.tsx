...
function Notice({ state }: { state: ActionState }) {
  ...
  return (
    <p
      className={`mt-3 rounded-md px-3 py-2 text-sm ${state.error ? "bg-err-soft text-err" : "bg-clay text-ok"}`}
      aria-live="polite"
    >
              ...
              <tbody className="divide-y divide-line-soft">
                {groupsWithHealth.map((group) => (
                  <tr key={group.id} className={group.id === selectedGroup?.id ? "bg-brand/10" : "bg-paper"}>
                    ...
                    <td className="px-4 py-4"><div className="flex justify-end gap-2"><button type="button" onClick={() => openDrawer("zone", group.id)} className="rounded-md border border-line px-2.5 py-1.5 text-xs font-semibold text-ink hover:bg-clay">Zones</button>{confirmingGroupId === group.id ? <form action={groupDropAction} className="flex gap-1.5"><input type="hidden" name="groupId" value={group.id} /><button disabled={groupDropPending} className="rounded-md bg-err px-2.5 py-1.5 text-xs font-bold text-white disabled:opacity-60">Confirmer</button><button type="button" onClick={() => setConfirmingGroupId(null)} className="rounded-md border border-line px-2.5 py-1.5 text-xs font-semibold text-ink-soft">Annuler</button></form> : <button type="button" onClick={() => setConfirmingGroupId(group.id)} className="rounded-md border border-line p-1.5 text-err hover:bg-err-soft" title="Supprimer le groupe"><Trash2 className="h-3.5 w-3.5" /></button>}</div></td>
                  </tr>
                ))}
