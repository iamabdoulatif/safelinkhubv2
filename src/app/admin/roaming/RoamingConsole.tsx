...
import { Eye, Layers3, MapPin, Pause, Pencil, Play, Plus, Ticket, Trash2, UserPlus, Wifi } from "lucide-react";
import {
...
export default function RoamingConsole({
  ...
  // Suppressions en DEUX temps, comme pour les comptes : le premier clic arme.
  ...
  // La section 05 a son PROPRE groupe : créer un compte de technicien pour une
  // zone pendant qu'on émet des tickets pour une autre est un cas courant, et
  // l'unique sélecteur partagé trompait — au chargement il pointait le premier
  // groupe, dont les offres n'étaient pas celles des comptes affichés dessous.
  ...
  return (
    <div className="animate-fade-in-up pb-8">
          ...
          <div className="mt-4 space-y-3">
            ...
            {groups.map((group) => (
              <article key={group.id} className="border border-line-soft p-4">
                <div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-semibold text-ink">{group.name}</h3><p className="font-mono text-xs text-ink-soft">{group.code}</p></div><form action={groupToggleAction}><input type="hidden" name="groupId" value={group.id} /><input type="hidden" name="active" value={group.active ? "false" : "true"} /><button disabled={groupTogglePending} title={group.active ? "Mettre ce groupe en pause : plus aucune émission, rien n'est retiré des MikroTik." : "Réactiver l'émission sur ce groupe."} className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold transition-colors disabled:opacity-60 ${group.active ? "bg-brand text-ink hover:bg-ink hover:text-paper" : "bg-clay text-ink-soft hover:bg-ink hover:text-paper"}`}>{group.active ? <Pause className="h-3 w-3" /> : <Play className="h-3 w-3" />}{group.active ? "Actif" : "En pause"}</button></form></div>
                ...
                {!group.active && <p className="mt-3 border-l-2 border-warn bg-clay/50 px-3 py-2 text-xs leading-5 text-ink-soft">Groupe en pause : aucun nouveau ticket ni compte ne peut être émis. Les comptes déjà posés sur les MikroTik continuent de fonctionner — et vous pouvez toujours les modifier ou les révoquer.</p>}<button type="button" onClick={() => setAddingZoneToGroupId(addingZoneToGroupId === group.id ? null : group.id)} aria-expanded={addingZoneToGroupId === group.id} className="mt-3 inline-flex items-center gap-1.5 rounded-md border border-line px-2.5 py-1.5 text-xs font-semibold text-ink hover:bg-clay"><Plus className="h-3.5 w-3.5" />Ajouter une zone</button>{confirmingGroupId === group.id ? <form action={groupDropAction} className="mt-3 ml-2 inline-flex items-center gap-1.5"><input type="hidden" name="groupId" value={group.id} /><button disabled={groupDropPending} className="rounded-md bg-red-600 px-2.5 py-1.5 text-xs font-bold text-white hover:bg-red-700 disabled:opacity-60">{groupDropPending ? "Suppression…" : "Confirmer la suppression"}</button><button type="button" onClick={() => setConfirmingGroupId(null)} className="rounded-md border border-line px-2.5 py-1.5 text-xs font-semibold text-ink-soft hover:bg-clay">Annuler</button></form> : <button type="button" onClick={() => { setConfirmingGroupId(group.id); setAddingZoneToGroupId(null); }} className="mt-3 ml-2 inline-flex items-center gap-1.5 rounded-md border border-line px-2.5 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" />Supprimer</button>}{confirmingGroupId === group.id && <p className="mt-2 border-l-2 border-red-600 bg-red-50 px-3 py-2 text-xs leading-5 text-red-800">Le groupe, ses {group.routers.length} zone(s) et ses offres seront supprimés. Les tickets déjà vendus gardent leur code mais perdent le lien vers ce groupe, et les profils posés sur les MikroTik ne sont pas retirés. La suppression est <strong>refusée</strong> tant qu&apos;il reste un compte nominatif : il deviendrait actif sur les routeurs sans plus aucun bouton pour le couper.</p>}
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[0.92fr_1.08fr]">
        <section className="border-2 border-line bg-paper p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-brand">03 — tarification</p><h2 className="mt-1 text-xl font-bold text-ink">Prix par groupe</h2></div><span className="rounded-full bg-clay px-2.5 py-1 text-xs text-ink-soft">hérité ou local</span></div><div className="mt-4 divide-y divide-line-soft border-y border-line-soft">{offers.map((offer) => <div key={offer.id} className="flex items-center justify-between gap-3 py-3"><div><p className="font-medium text-ink">{offer.groupName} <span className="font-mono text-xs text-ink-soft">/ {offer.profileName}</span></p><p className="mt-0.5 text-xs text-ink-soft">{offer.priceOverrideCents === null ? "Prix catalogue" : "Prix spécifique au groupe"}{!offer.active && <span className="ml-1.5 rounded-full bg-clay px-2 py-0.5 font-semibold text-ink-soft">en pause</span>}</p></div><div className="flex items-center gap-2"><strong className={`text-sm ${offer.active ? "text-ink" : "text-ink-soft line-through"}`}>{money(offer.effectivePriceCents)}</strong><form action={offerToggleAction}><input type="hidden" name="offerId" value={offer.id} /><input type="hidden" name="active" value={offer.active ? "false" : "true"} /><button disabled={offerTogglePending} title={offer.active ? "Mettre cette offre en pause : elle disparaît des choix d'émission, rien n'est retiré des MikroTik." : "Remettre cette offre en vente."} className="flex items-center gap-1.5 rounded-md border border-line px-2 py-1 text-xs font-semibold text-ink-soft hover:bg-clay disabled:opacity-60">{offer.active ? <><Pause className="h-3 w-3" />Pause</> : <><Play className="h-3 w-3" />Reprendre</>}</button></form>{confirmingOfferId === offer.id ? <form action={offerDropAction} className="flex items-center gap-1.5"><input type="hidden" name="offerId" value={offer.id} /><button disabled={offerDropPending} className="rounded-md bg-red-600 px-2 py-1 text-xs font-bold text-white hover:bg-red-700 disabled:opacity-60">{offerDropPending ? "…" : "Confirmer"}</button><button type="button" onClick={() => setConfirmingOfferId(null)} className="rounded-md border border-line px-2 py-1 text-xs font-semibold text-ink-soft hover:bg-clay">Annuler</button></form> : <button type="button" onClick={() => setConfirmingOfferId(offer.id)} title="Retirer cette offre du catalogue. Sans effet sur les MikroTik." className="rounded-md border border-line p-1.5 text-red-700 hover:bg-red-50"><Trash2 className="h-3 w-3" /></button>}</div></div>)}{offers.length === 0 && <p className="py-4 text-sm text-ink-soft">Aucune offre activée pour l’instant.</p>}</div><form action={offerAction} className="mt-5"><div className="grid gap-3 sm:grid-cols-2"><label className={labelClass}>Groupe<select name="groupId" required className={inputClass}><option value="">Choisir…</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label className={labelClass}>Profil<select name="profileId" required className={inputClass}><option value="">Choisir…</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name} — {money(profile.defaultPriceCents)}</option>)}</select></label></div><label className={`mt-3 ${labelClass}`}>Tarif du groupe <span className="normal-case tracking-normal">(laisser vide = catalogue)</span><input name="priceOverrideCents" inputMode="numeric" placeholder="Ex : 300" className={inputClass} /></label><button disabled={offerPending || groups.length === 0 || profiles.length === 0} className="mt-3 rounded-md border border-line bg-paper px-4 py-2 text-sm font-semibold text-ink hover:bg-clay disabled:opacity-60">{offerPending ? "Enregistrement…" : "Activer l’offre"}</button><Notice state={offerState} /></form><Notice state={offerToggleState} /><Notice state={offerDropState} /></section>
        ...
        <section className="border-2 border-line bg-paper p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-brand">05 — comptes nominatifs</p><h2 className="mt-1 text-xl font-bold text-ink">Créer un utilisateur</h2><p className="mt-1 text-sm text-ink-soft">Un identifiant et un mot de passe choisis, au lieu d’un code tiré au hasard — pour les administrateurs et techniciens de zone. Associé à une offre illimitée, le compte n’expire pas.</p></div><UserPlus className="h-6 w-6 text-brand" /></div>
          ...
          <form action={userAction} className="mt-5 border-t border-line-soft pt-5"><input type="hidden" name="groupId" value={userGroupId} /><label className={labelClass}>Groupe<select value={userGroupId} onChange={(event) => setUserGroupId(event.target.value)} className={inputClass}><option value="">Choisir…</option>{groups.filter((group) => group.active).map((group) => <option key={group.id} value={group.id}>{group.name} · {group.routers.length} zone(s)</option>)}</select></label><p className="mt-1.5 text-xs text-ink-soft">Le compte est posé sur <strong className="text-ink">toutes</strong> les zones de ce groupe.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className={labelClass}>Identifiant<input name="username" required maxLength={32} pattern={ROAMING_USERNAME_PATTERN} title="Lettres, chiffres, point, tiret, souligné ou arobase — 2 à 32 caractères." placeholder="ex : aroune" autoComplete="off" className={`${inputClass} font-mono`} /></label><label className={labelClass}>Mot de passe <span className="normal-case tracking-normal">(vide = identique)</span><input name="password" maxLength={64} placeholder="ex : aroune" autoComplete="off" className={`${inputClass} font-mono`} /></label></div><label className={`mt-3 ${labelClass}`}>Offre<select name="offerId" required disabled={!userGroupId || userCreationOffers.length === 0} className={inputClass}><option value="">{userCreationOffers.length ? "Choisir un profil…" : "Aucune offre active pour ce groupe"}</option>{userCreationOffers.map((offer) => <option key={offer.id} value={offer.id}>{offer.profileName} — {money(offer.effectivePriceCents)}</option>)}</select></label><label className={`mt-3 ${labelClass}`}>Rôle ou note<input name="note" maxLength={180} placeholder="ex : technicien zone nord" className={inputClass} /></label><button disabled={userPending || !userGroupId || userCreationOffers.length === 0} className="mt-4 rounded-md bg-ink px-5 py-2.5 text-sm font-bold text-paper hover:bg-brand disabled:opacity-60">{userPending ? "Création…" : "Créer l’utilisateur"}</button><Notice state={userState} /></form>
          ...
          {namedUsers.length > 0 && <div className="mt-5 border-t border-line-soft pt-4"><p className={labelClass}>Comptes existants</p><ul className="mt-2 divide-y divide-line-soft border-y border-line-soft">{namedUsers.map((user) => { const userOffers = offers.filter((offer) => offer.groupId === user.groupId && offer.active && offer.groupActive && offer.profileActive); return <li key={user.id} className="py-2.5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button type="button" onClick={() => { setEditingId(editingId === user.id ? null : user.id); setConfirmingId(null); setRevealed((prev) => { const next = { ...prev }; delete next[user.id]; return next; }); }} aria-expanded={editingId === user.id} className="flex items-center gap-1.5 rounded-md border border-line px-2.5 py-1.5 text-xs font-semibold text-ink hover:bg-clay"><Pencil className="h-3.5 w-3.5" />Modifier</button>
