...
const inputClass =
  "mt-1 w-full rounded-lg border border-line-soft bg-paper px-3 py-2 text-sm text-ink outline-none transition focus:border-brand";
const labelClass = "block text-xs font-semibold uppercase tracking-[0.12em] text-ink-soft";
const panelClass = "rounded-xl border border-line-soft bg-paper p-5 shadow-sm sm:p-6";
...
function RoamKpi({ value, label, tone }: { value: number; label: string; tone?: "ok" }) {
  return (
    <div className="rounded-xl border border-line-soft bg-paper px-4 py-3.5 shadow-sm">
      <p className="text-[26px] font-extrabold leading-none tracking-tight tabular-nums text-ink">{value}</p>
      <p className="mt-1.5 flex items-center gap-1.5 font-mono text-xs font-semibold uppercase tracking-[0.11em] text-ink-soft">
    ...
    <div className="animate-fade-in-up pb-10">
      <header className="flex flex-wrap items-end justify-between gap-6">
        <div>
          ...
          <h1 className="mt-2.5 max-w-[18ch] text-ink [text-wrap:balance] text-2xl font-semibold tracking-tight">
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            {item.count !== undefined && (
              <span className={`rounded-full px-1.5 py-0.5 font-mono text-xs font-bold tabular-nums ${activeView === item.id ? "bg-brand text-slate-deep" : "bg-clay text-ink-soft"}`}>
                {item.count}
              </span>
            )}
          </button>
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
          <div className={`${panelClass} min-h-[420px]`}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => openDrawer("tickets")} disabled={!selectedGroup?.active || selectableOffers.length === 0} className="inline-flex items-center gap-2 rounded-full bg-brand px-4 py-2.5 text-sm font-semibold text-slate-deep transition-colors hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50">
              ...
              <div className="mt-5">
                <div className="flex flex-wrap items-center justify-between gap-3 border-y border-line-soft py-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => openDrawer("zone", selectedGroup.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-line px-3 py-1.5 text-xs font-semibold text-ink hover:bg-clay"><Plus className="h-3.5 w-3.5" /> Zone</button>
                  </div>
                </div>
          ...
          <aside className="space-y-5">
            ...
            <section className={panelClass}>
              ...
              <button type="button" onClick={() => setActiveView("catalogue")} className="mt-5 w-full rounded-lg border border-line px-3 py-2 text-sm font-semibold text-ink hover:bg-clay">Gérer le catalogue</button>
            </section>
          </aside>
        </section>
      )}
      ...
      {activeView === "groups" && (
        <section className={`mt-5 ${panelClass}`}>
          ...
          {confirmingGroupId && <p className="mt-3 border-l border-err bg-err-soft px-3 py-2 text-xs leading-5 text-err">La suppression retire la structure et ses offres du SaaS, sans effacer les profils des MikroTik. Elle est refusée tant qu’un compte nominatif est encore rattaché à ce groupe.</p>}
      ...
      {activeView === "catalogue" && (
        <section className={`mt-5 ${panelClass}`}>
          <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-brand">Référentiel commercial</p><h2 className="mt-1 text-2xl font-bold text-ink">Catalogue</h2><p className="mt-1 text-sm text-ink-soft">Les profils sont communs ; les offres choisissent où et à quel prix ils sont émis.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => openDrawer("profile")} className="inline-flex items-center gap-2 rounded-lg border border-line px-3 py-2 text-sm font-semibold text-ink hover:bg-clay"><Plus className="h-4 w-4" /> Profil</button><button type="button" onClick={() => openDrawer("offer")} className="inline-flex items-center gap-2 rounded-lg bg-ink px-3 py-2 text-sm font-semibold text-paper hover:bg-brand"><Plus className="h-4 w-4" /> Offre</button></div></div>
          <div className="mt-6 grid gap-5 xl:grid-cols-[0.88fr_1.12fr]">
            ...
            <div><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold text-ink">Offres du groupe</h3><p className="mt-1 text-xs text-ink-soft">Tarif catalogue ou ajustement local.</p></div><select value={selectedGroup?.id ?? ""} onChange={(event) => setSelectedGroupId(event.target.value)} className="rounded-lg border border-line-soft bg-paper px-3 py-2 text-sm text-ink outline-none focus:border-brand">{groupsWithHealth.length === 0 && <option value="">Aucun groupe</option>}{groupsWithHealth.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></div><div className="mt-3 divide-y divide-line-soft border-y border-line-soft">{selectedOffers.map((offer) => <div key={offer.id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><p className="font-medium text-ink">{offer.profileName}</p><p className="mt-0.5 text-xs text-ink-soft">{offer.priceOverrideCents === null ? "Prix catalogue" : "Tarif spécifique au groupe"}{!offer.active && " · en pause"}</p></div><div className="flex items-center gap-2"><strong className={offer.active ? "text-ink" : "text-ink-soft line-through"}>{money(offer.effectivePriceCents)}</strong><form action={offerToggleAction}><input type="hidden" name="offerId" value={offer.id} /><input type="hidden" name="active" value={offer.active ? "false" : "true"} /><button disabled={offerTogglePending} className="rounded-lg border border-line px-2.5 py-1.5 text-xs font-semibold text-ink hover:bg-clay disabled:opacity-60">{offer.active ? "Pause" : "Reprendre"}</button></form>{confirmingOfferId === offer.id ? <form action={offerDropAction} className="flex gap-1.5"><input type="hidden" name="offerId" value={offer.id} /><button disabled={offerDropPending} className="rounded-lg bg-err px-2 py-1.5 text-xs font-bold text-white disabled:opacity-60">{offerDropPending ? "…" : "Confirmer"}</button><button type="button" onClick={() => setConfirmingOfferId(null)} className="rounded-lg border border-line px-2 py-1.5 text-xs font-semibold text-ink-soft">Annuler</button></form> : <button type="button" onClick={() => setConfirmingOfferId(offer.id)} className="rounded-lg border border-line p-1.5 text-err hover:bg-err-soft" title="Retirer cette offre"><Trash2 className="h-3.5 w-3.5" /></button>}</div></div>)}{selectedGroup && selectedOffers.length === 0 && <p className="py-5 text-sm text-ink-soft">Aucune offre pour ce groupe.</p>}{!selectedGroup && <p className="py-5 text-sm text-ink-soft">Créez un groupe avant d’y activer une offre.</p>}</div></div>
          </div>
      ...
      {activeView === "accounts" && (
        <section className={`mt-5 ${panelClass}`}>
          <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-brand">Accès d’équipe</p><h2 className="mt-1 text-2xl font-bold text-ink">Comptes</h2><p className="mt-1 text-sm text-ink-soft">Gérez les accès nominatifs sans exposer les mots de passe stockés sur les MikroTik.</p></div><button type="button" onClick={() => openDrawer("account")} className="inline-flex items-center gap-2 rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-paper hover:bg-brand"><UserPlus className="h-4 w-4" /> Nouveau compte</button></div>
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3"><p className={labelClass}>Comptes existants</p><label className="sr-only" htmlFor="account-search">Rechercher un compte</label><input id="account-search" value={accountSearch} onChange={(event) => setAccountSearch(event.target.value)} placeholder="Rechercher un compte…" className="w-full rounded-lg border border-line-soft bg-paper px-3 py-2 text-sm text-ink outline-none focus:border-brand sm:w-64" /></div>
          <ul className="mt-2 divide-y divide-line-soft border-y border-line-soft">
