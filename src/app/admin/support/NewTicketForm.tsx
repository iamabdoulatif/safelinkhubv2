...
export default function NewTicketForm() {
  ...
  return (
    <form ref={formRef} action={formAction} className="border border-line bg-paper p-6 rounded-xl">
      ...
      {state?.success && (
        <p className="mt-4 rounded-lg bg-clay px-3 py-2 text-sm text-ok">
          Demande envoyée.
        </p>
      )}
      ...
      <button
        type="submit"
        disabled={pending}
        className="mt-4 rounded-lg bg-ink px-4 py-2 text-sm font-medium text-white hover:bg-slate-deep-line disabled:opacity-60"
      >
