...
describe("removeDualWanConfig", () => {
  it("retire par clé (commentaire, marque, table, liste), remet WAN1 d'aplomb, rebranche WAN2", async () => {
    ...
    const data: Record<string, Record<string, string>[]> = {
      "/ip/firewall/mangle/print": [{ ".id": "*1", comment: "PCC 1/2 -> WAN1" }, { ".id": "*2", action: "mark-routing", "new-routing-mark": "to-WAN2" }, { ".id": "*3", action: "change-ttl" }, { ".id": "*4", comment: "PCC DNS local udp", action: "accept" }],
    ...
    assert.deepEqual(report, { mangle: 3, routes: 2, tables: 1, nat: 1, address_list: 1, wan_list: 1, dhcp_wan2: 1, dhcp_wan1_restored: 1, wan2_bridged: 1 });
    const removes = calls.filter((c) => c[0].endsWith("/remove")).map((c) => `${c[0]} ${c[1]}`);
    assert.deepEqual(removes, ["/ip/firewall/mangle/remove =numbers=*1,*2,*4", "/ip/route/remove =numbers=*a,*b", "/routing/table/remove =numbers=*t", "/ip/firewall/nat/remove =numbers=*n", "/ip/firewall/address-list/remove =numbers=*l", "/interface/list/member/remove =numbers=*w", "/ip/dhcp-client/remove =numbers=*d2"]);
