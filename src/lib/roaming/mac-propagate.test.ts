...
test("matérialise une liaison MAC en liant le code et en créant son compagnon", async () => {
  const calls: string[][] = [];
  const client = {
  ...
  assert.ok(
    calls.some((sentence) =>
      sentence[0] === "/ip/hotspot/user/set" && sentence.includes("=mac-address=AA:BB:CC:DD:EE:FF"),
    ),
  );
